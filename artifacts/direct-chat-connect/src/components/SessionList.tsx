...
export const PlatformBadge = ({ platform, size = 'md' }: { platform: Platform; size?: 'sm' | 'md' }) => {
  ...
  // md = used in session list (avatar 48px), sm = used in conversation header (avatar 40px)
  const outerCls = size === 'sm'
    ? 'absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center shadow-md border-2 border-background'
    : 'absolute -top-1.5 -right-1.5 w-6 h-6 rounded-full flex items-center justify-center shadow-md border-2 border-background';
  const iconSize = size === 'sm' ? 11 : 13;
  return (
    <span className={outerCls} style={{ background: bg }} title={cfg.label}>
      {platform === 'whatsapp' && (
